...
def test_model_input_image_is_one_bounded_normalized_jpeg():
    ...
    with pytest.raises(ValueError, match="limit"):
        ModelInputImage(
            mime_type="image/jpeg",
            data=b"\xff\xd8\xff" + b"0" * MAX_MODEL_IMAGE_BYTES,
        )
...
async def test_quote_attachment_strategy_only_describes_the_direct_reply() -> None:
    ...
    assert context.model_images == (describer.image,)


@pytest.mark.asyncio
async def test_handler_passes_the_directly_quoted_image_to_the_agent() -> None:
    transport = FakeTransport()
    gateway = FakeGateway()
...
@pytest.mark.asyncio
async def test_owner_standalone_group_request_keeps_self_customization():
    transport = FakeTransport()
    gateway = FakeGateway()
    codec = NamespacedIdentityCodec(
        source="qq",
