...
change Sidekick's runtime memory behavior.
...
```bash
uv sync
uv run python -m sidekick.memory_benchmark.cli --help
```
...
The exporter accepts Hindsight documents using Sidekick's
...
from sidekick.memory_benchmark.source import export_hindsight_bank, write_corpus
...
of four mirrors Sidekick's dream ingestion path.

```bash
uv run python -m sidekick.memory_benchmark.cli ingest-hindsight \
...
```bash
uv run python -m sidekick.memory_benchmark.cli wait-hindsight \
  --url http://127.0.0.1:18889 \
  --bank benchmark:example:run-1 \
...
```bash
uv run python -m sidekick.memory_benchmark.cli seed-tencent \
  --source .benchmark-data/source.json \
...
```bash
uv run python -m sidekick.memory_benchmark.cli generate-cases \
  --env-file .env \
  --model gpt-5.4-mini \
...
```bash
uv run python -m sidekick.memory_benchmark.cli validate-cases \
  --env-file .env \
  --model gpt-5.6-sol \
  --reasoning-effort low \
  --source .benchmark-data/source.json \
...
```bash
uv run python -m sidekick.memory_benchmark.cli quality \
  --env-file .env \
  --model gpt-5.4-mini \
  --reasoning-effort low \
  --source .benchmark-data/source.json \
...
```bash
uv run python -m sidekick.memory_benchmark.cli report \
...
Hindsight was the better fit for Sidekick's recall-first chat experience in
