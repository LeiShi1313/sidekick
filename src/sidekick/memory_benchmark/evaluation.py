...
from sidekick.memory_benchmark.backends import MemoryRecord
from sidekick.memory_benchmark.source import SourceCorpus, SourceDocument
...
def write_cases(cases: Iterable[RecallCase], path: Path) -> None:
    _write_json(
        path,
        {
            "schema": "sidekick.memory-benchmark.recall-cases.v1",
...
def read_cases(path: Path) -> tuple[RecallCase, ...]:
    ...
    if value.get("schema") != "sidekick.memory-benchmark.recall-cases.v1":
