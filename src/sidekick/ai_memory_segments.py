    ...
    def from_env(cls) -> MemorySegmentationSettings:
        return cls(
            idle_gap=timedelta(
                seconds=float(
                    os.environ.get(
                        "SIDEKICK_MEMORY_SEGMENT_IDLE_SECONDS",
                        "300",
                    )
                )
            ),
            max_span=timedelta(
                seconds=float(
                    os.environ.get(
                        "SIDEKICK_MEMORY_SEGMENT_MAX_SPAN_SECONDS",
                        "3600",
                    )
                )
            ),
            max_events=int(
                os.environ.get("SIDEKICK_MEMORY_SEGMENT_MAX_EVENTS", "30")
            ),
            max_chars=int(
                os.environ.get("SIDEKICK_MEMORY_SEGMENT_MAX_CHARS", "4000")
            ),
        )
